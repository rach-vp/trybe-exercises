...
Before beginning, create HTML and CSS files in your directory and copy and paste the respective codes below:
...
Start the exercise analysing the page in a small screen, to simulate how would it look like in a mobile device.
Now you're going to adjust the CSS to improve the page visualization.
Follow these steps:

- Increase the font size;
- Decrease the font size of ``h1`` elements;
- Increase the spacing between pictures;
- Add some margin to the page.

> Thinking about a small screen design first, we are applying an approach called _mobile first_. Doing it, we're starting with a basic design and then work on more sophisticated improvements for browsers with more advanced properties and specific layouts.
> It guarantees that we are developing an experience that will work for everyone. It also has collateral damage: helps us to notice which content really matters to the page.

Start expanding the screen width. Do it until a breakpoint where the actual design doesn't work anymore.

- For example, The width of the lines may look too big to the text to be easily read. Or maybe, the width is too big that doesn't make sense showing the images in a column.

Do the following tasks:

- Save the screen width that you identified that the layout doesn't look great. It will be the first breakpoint. A breakpoint is a point where is defined for the actual layout to change.
- Create a media query in your CSS file using the dimensions in pixels that you've saved as the ``min-width`` of the media query test. Inside this breakpoint, add the following adjustments;

  - Change the background colour (it'll help to notice when the media query has an effect);
  - Adjust the font size;
  - Adjust the margins;
  - Make the images to be shown in two columns.

Now, you're going to create another _breakpoint_ for big screens. Resize your screen again to find a new one.

- Create a new media query in your CSS file using the dimensions in pixels that you've saved for the big screens and follow these adjustments:

  - Change the background colour;
  - Change the font size;
  - Adjust the margins;
  - Add the property ``max-width`` in order to assure that the lines aren’t too big.

## Part II

Before starting, copy the template HTML and CSS listed below and save in their respective file:
...
>The page above is a no-style version of the first chapter of the book: _ALice in Wonderland_.

- Using the mobile-first approach and media queries, create three layouts for this page. Each one must correspond to screen size (small, mid and big).

  - Make at least one commit per layout.

- To do this properly, you must pay attention to the page content and think about how to show it in each screen size:

  - Where the list of chapters will be positioned?
  - How the story should be shown?
  - How the header should be positioned?
  - What to do with the author's info in each screen size?

  - You may need to change the HTML a little, add some elements to ease the styling, or maybe change some block positioning.
...
The aim of this exercise is to use media queries to make the page looks different when printing it and in different screen sizes.

- Add a media query and the needed rules in order to:

  - Elements IDed as ``header``, ``navigation`` and ``footer`` to disappear;
  - Element IDed as ``aside`` to be shown below the main content.

- Add a media query and the needed rules in order to the page, when resized the smaller screens:

  - Element IDed as ``aside`` must disappear;
  - ``body`` is unpadded;
  - Images mustn't exceed the screen's width;
  - Other items inside ``navigation`` must appear one in each line;
  - Element IDed as ``header`` must be fixed in a way that it always appears on the top of the page as the user scrolls.
